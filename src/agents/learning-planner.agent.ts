...
import { LearningPlanRepository } from "../repositories/learning-plan.repository.ts"
...
export class LearningPlannerAgent implements AgentHandler<Payload, LearningPlan> {
  ...
  private readonly repo: LearningPlanRepository
  constructor(db: InMemoryDatabase) { this.repo = new LearningPlanRepository(db) }
  async run(task: AgentTaskInput<Payload>): Promise<AgentTaskOutput<LearningPlan>> {
    ...
    const plan = this.repo.get(task.payload.learningPlanId)
